...
def _initialize_config():
    ...
    CONFIG_VARIABLES = [
        ...
        # Domain-specific
        "VALIDATION_CONFIG_PATH", "VERSION",
        # LLM Models (for observability cost tracking)
        "LLM_MODELS",
        # Content Safety
        "CONTENT_SAFETY_ENABLED", "CONTENT_SAFETY_SEVERITY_THRESHOLD",
    ]

    # Add Azure AI Search variables (always from .env)
    CONFIG_VARIABLES += AZURE_SEARCH_VARS
    ...
    for var_name in CONFIG_VARIABLES:
        ...
        if var_name == "OBS_AZURE_SQL_TRUST_SERVER_CERTIFICATE":
            if not value:
                value = "yes"  # Default for Azure SQL trust server certificate
        ...
        # LLM_MODELS is expected as JSON string or list
